...
function BillingAddresss({getGst}) {
  ...
  const getState = () => {
    ...
    for (let i = 0; i < state_arr.length; i++) {
      newArr.push({id: state_arr[i].code, name: state_arr[i].name});
    }
    console.log(newArr);
    setStateData(newArr);
  };
  ...
  const renderFieldCity = settings => {
    const {selectedItem, defaultText, getLabel, clear} = settings;
    return (
      <View style={styles.container}>
        <View>
          {!selectedItem && (
            <Text style={[styles.text, {color: "#000000"}]}>{city ? city : "--Select City--"}</Text>
          )}
          {selectedItem && (
            <View style={{}}>
              <Text style={[styles.text, {color: selectedItem.color}]}>
  ...
  const setCityD = text => {
    setCity(text.name);
    dispatchAction(updateBilling({...user.billing, city: text.name}));
  };
  ...
  const getCity = state_id => {
    ...
    let newArr = [];
    for (let i = 0; i < state_arr.length; i++) {
      if (state_arr[i].code == state_id) {
        for (let j = 0; j < state_arr[i].districts.length; j++) {
          newArr.push({id: state_arr[i].districts[j].id, name: state_arr[i].districts[j].name});
        }
      }
    }
    console.log(newArr);
      ...
      <ScrollView style={{paddingHorizontal: 16}}>
        ...
        <>
          ...
          <CustomPicker
            ...
            getLabel={item => item.name}
            optionTemplate={renderOption}
            fieldTemplate={renderFieldCity}
            onValueChange={value => setCityD(value)}
          />
        </>
